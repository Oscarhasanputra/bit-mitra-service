...
@Component
@Slf4j
public class UpdateMsBankCommandReactiveImpl implements UpdateMsBankCommandReactive {
    @Autowired
    ...
    @Override
    public Mono<List<BaseResponseDTO>> execute(List<MsBankUpdateRequestDTO> requests, ModuleCodeEnum module, CrudCodeEnum crud, MandatoryHeaderRequestDTO mandatoryHeaderRequestDTO) {
        return Mono.just(requests)
                .flatMap((requestDto)-> ReactiveSecurityContextHolderData.assignContextData(this.commandExecutor.executeAsReactive(UpdateMsBankCommand.class,requestDto, module, crud,mandatoryHeaderRequestDTO)))
