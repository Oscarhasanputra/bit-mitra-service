...
@Component
@Slf4j
public class GetListMsBankCommandReactiveImpl implements GetListMsBankCommandReactive {

    @Autowired
    private QMsBankRepository qMsBankRepository;

    @Override
    public Mono<CustomPageImpl<MsBankViewDTO>> execute(SearchRequestDTO request, ModuleCodeEnum module, CrudCodeEnum crud, MandatoryHeaderRequestDTO mandatoryHeaderRequestDTO) {
        return Mono.fromCallable(()->{
            Map<String, String> invalidFilterColumn = EnumColumnFilterBy.getInvalidFilterColumnType(MsBankSearchField.class, request.getFilterBy(), request.getSortBy());
