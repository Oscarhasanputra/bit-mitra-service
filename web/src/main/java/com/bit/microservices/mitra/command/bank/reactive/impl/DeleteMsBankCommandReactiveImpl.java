...
@Component
@Slf4j
public class DeleteMsBankCommandReactiveImpl implements DeleteMsBankCommandReactive {
    ...
    public Mono<List<BaseResponseDTO>> execute(List<DeleteRequestDTO> requests, ModuleCodeEnum module, CrudCodeEnum crud, MandatoryHeaderRequestDTO mandatoryHeaderRequestDTO) {
        return Mono.just(requests)
                .flatMap((requestDto)-> ReactiveSecurityContextHolderData.assignContextData(this.commandExecutor.executeAsReactive(DeleteMsBankCommand.class,requestDto, module, crud,mandatoryHeaderRequestDTO)))
