...
import com.bit.microservices.mitra.command.port.reactive.DeletePortCommandReactive;
...
@Component
@Slf4j
public class DeletePortCommandReactiveImpl implements DeletePortCommandReactive {
    ...
    @Override
    public Mono<List<BaseResponseDTO>> execute(List<DeleteRequestDTO> requests, ModuleCodeEnum module, CrudCodeEnum crud, MandatoryHeaderRequestDTO mandatoryHeaderRequestDTO) {
        return Mono.just(requests)
                .flatMap((requestDto)-> ReactiveSecurityContextHolderData.assignContextData(this.commandExecutor.executeAsReactive(DeletePortCommand.class,requestDto, module, crud,mandatoryHeaderRequestDTO)))
