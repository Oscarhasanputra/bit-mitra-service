...
import com.bit.microservices.mitra.command.port.reactive.GetListPortCommandReactive;
...
@Component
@Slf4j
public class GetListPortCommandReactiveImpl implements GetListPortCommandReactive {

    @Autowired
    private QMsPortRepository qMsPortRepository;

    @Override
    public Mono<CustomPageImpl<MsPortViewDTO>> execute(SearchRequestDTO request, ModuleCodeEnum module, CrudCodeEnum crud, MandatoryHeaderRequestDTO mandatoryHeaderRequestDTO) {
        return Mono.fromCallable(()->{
            Map<String, String> invalidFilterColumn = EnumColumnFilterBy.getInvalidFilterColumnType(MsPortSearchField.class, request.getFilterBy(), request.getSortBy());
            ...
            CustomPageImpl<MsPortViewDTO> response;

            QMsPort qMsPort = QMsPort.msPort;
            Pageable pageable = PageableUtils.pageableUtils(request);

            BooleanBuilder querySearch = FilterByBooleanExpression.constructPredicate(request,qMsPort, QMsPort.class, MsPortSearchField.class);

            try {
                Page<MsPortViewDTO> page = qMsPortRepository.findAll(querySearch, pageable);
